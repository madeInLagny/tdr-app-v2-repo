...
class MultiSelect {
  ...
  _template() {
    ...
    for (let i = 0; i < this.data.length; i++) {
      ...
      optionsHTML += `
        ...
        this.data[i].value
      }" role="option" aria-selected="${isSelected}" tabindex="-1">
    ...
    let template = `
      ...
      this.options.dropdownHeight
        ...
        : ""
                    ...
                    ${
                      this.options.search
                        ? '<input type="text" class="multi-select-search" placeholder="Search..." role="searchbox">'
                        : ""
                    }
                    ${selectAllHTML}
                    ${optionsHTML}
  ...
  _eventHandlers() {
    ...
    const toggleDropdown = (forceClose = false) => {
      ...
      if (
        forceClose ||
        headerElement.classList.contains("multi-select-header-active")
      ) {
      ...
      search.oninput = () => {
        this.element
          ...
          .forEach((option) => {
            ...
            option.style.display = text.includes(search.value.toLowerCase())
              ? "flex"
              : "none";
          });
      };
    ...
    optionsContainer.addEventListener("keydown", (e) => {
      ...
      if (currentFocused.closest(".multi-select-options")) {
        if (["ArrowDown", "ArrowUp"].includes(e.key)) {
          ...
          currentFocused.matches('[role="option"]')
        ) {
          e.preventDefault();
  ...
  _updateHeader() {
    this.element
      .querySelectorAll(
        ".multi-select-header-option, .multi-select-header-placeholder"
      )
